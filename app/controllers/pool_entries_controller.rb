class PoolEntriesController < ApplicationController
  ...
  before_action :verify_any_user, only: [:index, :create, :index_even_knocked_out, :destroy, :pool_entries_and_picks, :update]
  ...
  def create
    ...
    if @week.week_number != 1
      respond_to do | format |
        ...
        @pool_entry.errors.each do |error|
  ...
  def pool_entries_and_picks
    ...
    if @web_state.open_for_picks == false
      ...
      unless @my_active_pool_entries.present? 
        error_message = "No more picks remaining, come back next year!"
        render :json => {:error => error_message}, :status => :bad_request
      else
